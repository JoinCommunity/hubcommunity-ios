...
public class GraphQLClient: GraphQLClientProtocol, @unchecked Sendable {
    ...
    public func fetchEvents() async throws -> [Event] {
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw GraphQLError.networkError(NSError(domain: "GraphQL", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid response type"]))
        }
        
        guard httpResponse.statusCode == 200 else {
            throw GraphQLError.networkError(NSError(domain: "GraphQL", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: "HTTP \(httpResponse.statusCode)"]))
        }
        ...
        return eventsData.compactMap { eventData -> Event? in
            guard let id = eventData.id,
                  let title = eventData.title else { return nil }
            
            let tags = eventData.tags?.compactMap { tagData -> Tag? in
                guard let tagId = tagData.id,
                      let value = tagData.value else { return nil }
                return Tag(id: tagId, value: value)
            } ?? []
            
            let talks = eventData.talks?.compactMap { talkData -> Talk? in
                guard let talkId = talkData.id,
                      let talkTitle = talkData.title else { return nil }
            ...
            let communities = eventData.communities?.compactMap { communityData -> Community? in
                guard let communityId = communityData.id,
                      let communityTitle = communityData.title else { return nil }
                return Community(
                    id: communityId,
...
public class MockGraphQLClient: GraphQLClientProtocol, @unchecked Sendable {
    ...
    public init(mockEvents: [Event] = [], shouldThrowError: Bool = false, mockError: Error = GraphQLError.noData) {
